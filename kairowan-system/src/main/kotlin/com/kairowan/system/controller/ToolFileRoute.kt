 ...
 * @date 2026-01-30
 */
class ToolFileController : KController() {
    ...
    fun Route.routes() {
        ...
        route("/tool/file") {
            ...
            // 批量删除文件
            delete("/batch") {
                val params = call.receive<Map<String, List<Long>>>()
                val ids = params["ids"] ?: return@delete call.respond(KResult.fail<Any>("参数错误"))
...
fun Route.toolFileRoutes() {
    ToolFileController().apply { routes() }
}
